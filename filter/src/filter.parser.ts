...
export class FilterParser<TDto, TWhereInput> {
  ...
  public generateQueryFindOptions(
    ...
    const where = this.generateWhere(filterDto.filter, filterDto.orFilter);
  ...
  private generateWhere(
    ...
    // Hanlde orFilter (OR logic)
    for (const filterEntry of orFilter) {
      const fieldName = filterEntry.field;
      ...
      if (dbFieldName.length > 0 && dbFieldName[0] === "!") continue;

      const dbFieldNameParts = dbFieldName.split(".");
      let orCondition: any = Object.create(null);
      let currentWhere: any = orCondition;

      for (const dbFieldPart of dbFieldNameParts.slice(0, -1)) {
        currentWhere[dbFieldPart] = Object.create(null);
        currentWhere = currentWhere[dbFieldPart];
      }
